 ...
 *
 * 功能（按已确认计划）：
 *   - PA12 脉冲计数(TIM1/ETR) -> 每秒 CPS(8s 平滑)
 *   - 剂量率 = CPS x K(默认 0.0018, 待标定) ；累计剂量每秒累加(不清零)
 *   - OLED(SSD1306, PB6/PB7 IIC) 显示 RATE/CPS/ACC/THR/TIME/ALM
 *   - 按键(PA6/PA7/PB8/PB9)调阈值与累计时间
 *   - 超阈值声光报警(PB5 LED 闪烁 + PB0 蜂鸣器 PWM，越超越急促)
